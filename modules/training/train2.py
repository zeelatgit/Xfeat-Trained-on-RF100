...
class Trainer():
    def __init__(self, megadepth_root_path, rf100_root_path, synthetic_root_path, ckpt_save_path, model_name='xfeat_default',
                 batch_size=10, n_steps=160_000, lr=3e-4, gamma_steplr=0.5, training_res=(800, 608), device_num="0",
                 dry_run=False, save_ckpt_every=500, resume_ckpt=None):
        ...
        if model_name in ('xfeat_default', 'xfeat_synthetic'):
            ...
            train_data_synthetic, temp_data_synthetic = train_test_split(self.augmentor.train, test_size=0.3, random_state=42)
            test_data_synthetic, val_data_synthetic = train_test_split(temp_data_synthetic, test_size=1/3, random_state=42)

            self.train_loader_synthetic = DataLoader(train_data_synthetic, batch_size=int(self.batch_size * 0.4 if model_name == 'xfeat_default' else batch_size), shuffle=True)
            self.test_loader_synthetic = DataLoader(test_data_synthetic, batch_size=int(self.batch_size * 0.4 if model_name == 'xfeat_default' else batch_size), shuffle=False)
            self.val_loader_synthetic = DataLoader(val_data_synthetic, batch_size=int(self.batch_size * 0.4 if model_name == 'xfeat_default' else batch_size), shuffle=False)

            self.data_iter_synthetic = iter(self.train_loader_synthetic)
        else:
            self.augmentor = None
        ##################### Synthetic COCO END #######################
        ...
        if model_name in ('xfeat_default', 'xfeat_megadepth'):
            ...
            data_md = torch.utils.data.ConcatDataset([MegaDepthDataset(root_dir=TRAINVAL_DATA_SOURCE,
                                                                       npz_path=npz_path) for npz_path in npz_paths])

            # Split the dataset into train, test, and validation sets
            train_data_md, temp_data_md = train_test_split(data_md, test_size=0.3, random_state=42)
            test_data_md, val_data_md = train_test_split(temp_data_md, test_size=1/3, random_state=42)

            self.train_loader_md = DataLoader(train_data_md, batch_size=int(self.batch_size * 0.6 if model_name == 'xfeat_default' else batch_size), shuffle=True)
            self.test_loader_md = DataLoader(test_data_md, batch_size=int(self.batch_size * 0.6 if model_name == 'xfeat_default' else batch_size), shuffle=False)
            self.val_loader_md = DataLoader(val_data_md, batch_size=int(self.batch_size * 0.6 if model_name == 'xfeat_default' else batch_size), shuffle=False)

            self.data_iter_md = iter(self.train_loader_md)
        else:
    ...
    def evaluate(self, loader):
        ...
        # Check the size of validation set
        print(f"Validation set size (number of batches): {len(loader)}")
        ...
        with torch.no_grad():
            for d_md in loader:
                ...
                feats1, kpts1, hmap1 = self.net(p1_md)
                feats2, kpts2, hmap2 = self.net(p2_md)
                ...
                for b in range(len(positives_md_coarse)):
                    pts1, pts2 = positives_md_coarse[b][:, :2], positives_md_coarse[b][:, 2:]
                    ...
                    total_loss += loss_ds.item() + loss_coords.item()
                    total_accuracy += acc_coords
                    num_batches += 1
            ...
            for i in range(self.start_step, self.steps):
                ...
                with torch.inference_mode():
                    ...
                    if self.model_name in ('xfeat_default'):
                        p1 = torch.cat([p1s, p1_md], dim=0)
                        p2 = torch.cat([p2s, p2_md], dim=0)
                ...
                if (i + 1) % self.save_ckpt_every == 0:
                    self.evaluate(self.val_loader_synthetic)
